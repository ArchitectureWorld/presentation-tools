...
| P0-03 | 附件字节可落入 Canonical / Agent context | Blob ObjectStore、受控 asset API、projection scrub | `7ef4cf5`, `de19cef`, `71294f7`, `7dc05e3` | 20MB、export hash、context scrub tests | verified |
| P0-04 | Canonical 中存在竞争副本和身份漂移 | Canonical projection、stable IDs、Adapter 轮转 | `18f9d2c`, `a0c8d03`, `b2e9aa9`, `2adca59` | canonical / adapter regression tests | verified |
| P0-05 | export staging 可能覆盖或留下半成品 | unique staging、claim/final publish、失败清理 | `4e50e76`, `24df691`, `65afcfc`, `c7166fe` | standard export tests | verified |
| P0-06 | dirty edit、stale save、no-op Revision 语义不可靠 | page buffer、flush guard、CAS/no-op、caption save | `684b500`, `b5bd72f`, `d0bc441` | editor/server/domain regressions + 6 viewport browser gate | verified |
...
| P1-02 | vendor 和包来源可能漂移 | prepack sync、vendor manifest、current-HEAD tarball integrity | `627bb7f`, `3fa7312`, `1676abd`, `6b93b12` | clean vendor zero-diff + 39-file tarball integrity + Linux artifact | host-and-CI-verified |
...
1. 真实目标 Provider 的 `ReviewSubmission → studio_get_context → studio_apply_commands → Proposal → 接受 → 新 Revision → 重启恢复`。

## 2026-09-04 当前宿主证据

- 已部署插件源码截至 `1676abd3737e6fda53bd97918c5b7c2d746bc178`；最终门禁提交为 `6b93b1262f9fd636ea1fbe5df1da258857325851`。最新 GitHub push/PR 的 Linux、Windows 均完成 175/175、Contract、E2E、6 viewport UI、DSH smoke、release integrity 与 vendor zero-diff。
...
- 最终验证提交 `6b93b12`：push run [`33853542537`](https://github.com/ArchitectureWorld/presentation-tools/actions/runs/33853542537) 与 PR run [`33853544154`](https://github.com/ArchitectureWorld/presentation-tools/actions/runs/33853544154) 均为 Linux/Windows success；Standard Project run [`33853544298`](https://github.com/ArchitectureWorld/presentation-tools/actions/runs/33853544298) success。
- main branch protection 已回读：`strict=true`，required checks 为 `linux-verification`、`windows-verification`；force push、branch deletion 与 branch lock 均未启用。PR #6 保持 Draft，未合并。
- Linux CI artifact：39 files / 87,894 bytes / SHA-256 `1de7d5aabe7518126c5c3198738fd0e8b395e7fcb59e0c5a640328702c94750d`。与已部署 Windows 包逐文件比较，除 CRLF/LF 外无内容差异，因此未中断正在使用的 3080 服务重复安装。
