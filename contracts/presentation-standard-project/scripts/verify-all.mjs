...
process.stdout.write(run('python3', [path.join(packageRoot, 'scripts/verify-json-schema.py')]))
...
const count = tests.match(/# tests (\d+)/)?.[1] ?? 'unknown'
requireCondition(tests.includes('# fail 0'), 'Node tests failed')
console.log(`nodeTests=PASS tests=${count}`)

const packResult = JSON.parse(run('npm', ['pack', '--dry-run', '--json', '--ignore-scripts'], { cwd: packageRoot }))
...
try {
  const packed = JSON.parse(run('npm', ['pack', '--json', '--ignore-scripts', '--pack-destination', temp], { cwd: packageRoot }))
