...
test('body selection creates an anchored annotation after saving pending text', async () => {
  const app = await loadBrowser();
  const body = app.elements.get('#draft-body');
  body.value = '保留前文选中内容保留后文'; body.selectionStart = 4; body.selectionEnd = 8;
  await app.emit('input', { target: body });
  await app.emit('click', { target: target({ dataset: { commentDraftField: 'body' }, matches: ['[data-comment-draft-field]'] }) });
  app.elements.get('#annotation-input').value = '压缩这句话';
  await app.emit('click', { target: target({ id: 'add-annotation' }) });
  const saved = app.getState(); const annotation = saved.annotations[0];
  assert.ok(annotation, 'the annotation must be saved');
  assert.equal(annotation.target.type, 'draft-text');
  assert.equal(annotation.target.pageId, saved.pages[0].id);
  assert.equal(annotation.target.blockId, saved.pages[0].contentBlocks.find(block => block.type === 'text' && block.role !== 'title').contentBlockId);
  assert.deepEqual(annotation.target.selection, { start: 4, end: 8, quote: '选中内容' });
  assert.match(app.elements.get('#draft-stage').innerHTML, /正文段落/);
  assert.match(app.elements.get('#draft-stage').innerHTML, /正文要点/);
});

test('a single list item annotation carries the stable item and block identity', async () => {
  let initialState = stateWithPages(); const pageId = initialState.pages[0].id;
  initialState = executeAction(initialState, { type: 'draft.list.insert', pageId, content: '目标要点' }).state;
  const list = initialState.pages[0].contentBlocks.find(block => block.type === 'list'); const item = list.items[0];
  const app = await loadBrowser({ initialState });
  await app.emit('click', { target: target({ dataset: { commentListItem: item.listItemId }, matches: ['[data-comment-list-item]'] }) });
  app.elements.get('#annotation-input').value = '只修改这一条';
  await app.emit('click', { target: target({ id: 'add-annotation' }) });
  const annotation = app.getState().annotations[0];
  assert.equal(annotation.target.type, 'draft-list-item');
  assert.equal(annotation.target.id, item.listItemId);
  assert.equal(annotation.target.pageId, pageId);
  assert.equal(annotation.target.blockId, list.contentBlockId);
});
...
test('the page asset panel renders canonical pageAssets when the legacy assets alias is absent', async () => {
  const initialState = stateWithPages(); const page = initialState.pages[0]
  page.pageAssets = [{ pageAssetId: 'pageAsset_ai', assetId: 'asset_ai', name: 'AI 概念候选图', mimeType: 'image/png', role: 'supporting', caption: '', order: 0, sourceRefs: [] }]
  page.assets = []
  const app = await loadBrowser({ initialState })
  assert.match(app.elements.get('#draft-stage').innerHTML, /AI 概念候选图/)
  assert.match(app.elements.get('#draft-stage').innerHTML, /\/api\/assets\/asset_ai\/content/)
})

test('the page asset panel exposes an unadopted AI visual proposal as a read-only candidate', async () => {
  const initialState = stateWithPages(); const page = initialState.pages[0]
  page.assets = []; page.pageAssets = []
  initialState.proposals.push({
    id: 'proposal_visual_pending', kind: 'design.visual.v1', status: 'ready', pageId: page.id,
    objectRef: { mimeType: 'image/png', sha256: 'a'.repeat(64) }, message: '图片已生成，等待挂接',
  })
  const app = await loadBrowser({ initialState })
  const html = app.elements.get('#draft-stage').innerHTML
  assert.match(html, /本页视觉候选/)
  assert.match(html, /候选已生成/)
  assert.match(html, /页面 AI 概念视觉候选/)
  assert.match(html, /\/api\/design\/image\?proposalId=proposal_visual_pending/)
  assert.doesNotMatch(html, /data-remove-asset="proposal_visual_pending"/)
})
