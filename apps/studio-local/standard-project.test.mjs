...
const layoutFixtureRoot = resolve('contracts/presentation-standard-project/examples/unformatted-project/project_01992a80-0000-7000-8000-000000000101-campus-renewal-brief')
...
test('standard export blocks pages without a saved, blocker-free PNG layout preview', async t => {
  const {fixture}=await import('../../test-support/design-fixture.mjs')
  const fx=await fixture();t.after(()=>fx.close())
  await assert.rejects(createStandardProjectService(fx.repository,{layoutService:fx.layoutService}).exportProject(),error=>error.code==='standard_export_layout_blocked'&&error.statusCode===409&&error.details.pages.length>0)
})

test('standard export gate survives repository reopen when every page has a matching preview', async t => {
  const {fixture,inputForPage,applyCandidate}=await import('../../test-support/design-fixture.mjs')
  const fx=await fixture();t.after(()=>fx.close())
  const run=await fx.start({pageIds:fx.repository.getState().pages.map(p=>p.id),protectedPageIds:[],allowApply:true})
  for(const pageId of run.pageIds) await applyCandidate(fx,await fx.service.prepare(await inputForPage(fx,run,pageId)))
  await fx.reopen()
  const result=await createStandardProjectService(fx.repository,{layoutService:fx.layoutService}).exportProject()
  assert.equal(result.validation.valid,true)
})
