...
test('design operational metadata survives reopen without Canonical writes and rejects binary or unbounded values', async () => {
  const root = await mkdtemp(join(tmpdir(), 'studio-design-metadata-'))
  let repository = await createRepository(root)
  try {
    const before = await readFile(repository.controlPath, 'utf8')
    await repository.close()
    repository = await createRepository(root)
    assert.deepEqual(repository.getState().designRuns, [])
    assert.deepEqual(repository.getState().layoutCandidates, [])
    assert.equal(await readFile(repository.controlPath, 'utf8'), before)
    for (const value of [Buffer.from('binary'), { png: 'raw' }, { score: NaN }, { raw: 'x'.repeat(2_000_001) }]) {
      await assert.rejects(repository.transactOperational(state => { state.layoutCandidates = [value]; return state }), { code: 'invalid_command' })
    }
    assert.equal(await readFile(repository.controlPath, 'utf8'), before)
    await repository.transactOperational(state => { state.designRuns = [{ runId: 'host-run', pageIds: [], status: 'active' }]; state.layoutCandidates = [{ candidateId: 'candidate', objectRef: { sha256: 'a'.repeat(64) } }]; return state })
    await repository.close()
    repository = await createRepository(root)
    assert.equal(repository.getState().project.currentRevision, 0)
    assert.equal(repository.getState().designRuns[0].runId, 'host-run')
    assert.equal(repository.getState().layoutCandidates[0].objectRef.sha256, 'a'.repeat(64))
    assert.equal(JSON.stringify(await repository.getSnapshotAt(0)).includes('host-run'), false)
