...
async function loadBrowserApp({ fetchImpl, initialState: stateOverride } = {}) {
  ...
  const initialState = stateOverride ?? {
...
test('native iframe keeps the FAB visible and sizes the Agent dialog to about 80 percent', async () => {
  ...
  const height = card.includes('height: min(82dvh') ? 82 : Number(card.match(/(?:^|\n)\s*height:\s*(\d+)dvh/)?.[1]);
...
test('annotation task feed renders one aggregate card and never renders review Proposal details', async () => {
  const state = {
    project: { id: 'project_test', title: '测试项目', currentRevision: 3 }, ui: { stage: 'outline', activePageId: null }, outline: [], pages: [], annotations: [], reviewRounds: [],
    reviewSubmissions: [{ id: 'submission_1', reviewRoundId: 'round_1', number: 1, baseRevision: 2, status: 'accepted', createdAt: '2026-09-04T00:00:00.000Z' }],
    reviewRuns: [{ reviewRunId: 'run_1', reviewSubmissionId: 'submission_1', taskId: 'task_1', phase: 'completed', integrationState: 'accepted', summary: '已直接应用修改，Revision 3', closedAt: '2026-09-04T00:00:02.000Z', createdAt: '2026-09-04T00:00:01.000Z' }],
    proposals: [{ id: 'proposal_1', submissionId: 'submission_1', status: 'accepted', message: '不应展示', createdAt: '2026-09-04T00:00:01.500Z' }],
  };
  const { listeners, elements } = await loadBrowserApp({ initialState: state });
  for (const listener of listeners.get('click') ?? []) await listener({ target: { id: '', closest: selector => selector === '#agent-fab' ? { id: 'agent-fab' } : null } });
  const feed = elements.get('#agent-feed').innerHTML;
  assert.equal((feed.match(/批注任务/g) ?? []).length, 1);
  assert.doesNotMatch(feed, /Agent 修改建议|Proposal|不应展示/);
  assert.match(feed, /已直接应用修改，Revision 3/);
});
...
test('failed task details are not repeated and retries stay inside their own task cards', async () => {
  const state = {
    project: { id: 'project_test', title: '测试项目', currentRevision: 3 }, ui: { stage: 'outline', activePageId: null }, outline: [], pages: [], annotations: [], reviewRounds: [], proposals: [],
    reviewSubmissions: [1, 2, 3].map(n => ({ id: `submission_${n}`, reviewRoundId: `round_${n}`, number: 1, status: 'apply_failed', createdAt: `2026-09-0${n}T00:00:00.000Z`, ...(n === 1 ? { supersededBy: 'submission_2' } : {}) })),
    reviewRuns: [1, 2, 3].map(n => ({ reviewRunId: `run_${n}`, reviewSubmissionId: `submission_${n}`, phase: 'failed', summary: `错误 ${n}`, lastError: `错误 ${n}`, ...(n === 3 ? { executionMode: 'native', executionModel: { provider: 'test-provider', model: 'observed-model' } } : {}) })),
  };
  const { elements, listeners } = await loadBrowserApp({ initialState: state });
  for (const listener of listeners.get('click') ?? []) await listener({ target: { id: '', closest: selector => selector === '#agent-fab' ? { id: 'agent-fab' } : null } });
  const feed = elements.get('#agent-feed').innerHTML;
  assert.equal((feed.match(/错误 3/g) ?? []).length, 1);
  assert.doesNotMatch(feed, /data-retry-submission="submission_1"/);
  assert.match(feed, /错误 2[\s\S]*?data-retry-submission="submission_2"[^]*?<\/div>/);
  assert.match(feed, /错误 3[\s\S]*?data-retry-submission="submission_3"[^]*?<\/div>/);
  assert.doesNotMatch(feed, /重试批注 #1/);
  assert.match(feed, /DSH 主会话[\s\S]*test-provider[\s\S]*observed-model/);
});
