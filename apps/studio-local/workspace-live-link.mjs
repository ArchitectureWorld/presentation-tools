...
export async function resolveWorkspaceRoot(value) {
  ...
  if (!input || !isAbsolute(input)) {
    throw workspaceError(ERROR_CODES.WORKSPACE_UNAVAILABLE, 'DSH Session Workspace 必须是本机绝对路径。', { workspaceRoot: input || null })
  }
  const requested = resolve(input)
  try {
    const requestedInfo = await lstat(requested)
    if (requestedInfo.isSymbolicLink()) throw workspaceError(ERROR_CODES.WORKSPACE_UNAVAILABLE, 'DSH Session Workspace 根目录不能是符号链接。', { workspaceRoot: requested })
    if (!requestedInfo.isDirectory()) throw workspaceError(ERROR_CODES.WORKSPACE_UNAVAILABLE, 'DSH Session Workspace 必须指向目录。', { workspaceRoot: requested })
    const canonical = await realpath(requested)
    const canonicalInfo = await lstat(canonical)
    if (!canonicalInfo.isDirectory()) throw workspaceError(ERROR_CODES.WORKSPACE_UNAVAILABLE, 'DSH Session Workspace 必须指向目录。', { workspaceRoot: requested })
    return canonical
  } catch (error) {
    if (error instanceof StudioError) throw error
    throw workspaceError(ERROR_CODES.WORKSPACE_UNAVAILABLE, '无法读取当前 DSH Session Workspace。', {
...
export async function readWorkspaceSnapshot(workspaceRoot, { putBlob } = {}) {
  ...
  try {
    ...
    if (!project.isFile()) {
      return workspaceStatus(ERROR_CODES.WORKSPACE_PROJECT_MISSING, root, {
        validation: { valid: false, errors: [{ code: ERROR_CODES.WORKSPACE_PROJECT_MISSING, filePath: 'project.json', instancePath: '', message: 'Workspace 根目录没有 project.json。' }], warnings: [] },
      })
    }
  } catch (error) {
  ...
  if (!validation.valid) return workspaceStatus(ERROR_CODES.WORKSPACE_CONTRACT_INVALID, root, { validation })
  ...
  try {
    ...
    if (error?.code === 'standard_contract_invalid') {
      return workspaceStatus(ERROR_CODES.WORKSPACE_CONTRACT_INVALID, root, {
