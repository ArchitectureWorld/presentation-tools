import { escapeHtml, elementLabel, renderLayoutElements } from './layout-renderer.js'
...
function inspectorHtml(element) {
  ...
  return `
    ...
    <p><button data-layout-element-lock type="button">${record.protection?.elements?.some(lock=>lock.layoutElementId===element.layoutElementId)?'解除元素锁定':'锁定此元素'}</button></p>
    <label><input type="checkbox" data-layout-lock-on-save ${lockEditedElement?'checked':''}> 保存修改时锁定此元素</label>
...
function render(message = statusMessage) {
  ...
  if (!record?.layout) {
    root.innerHTML = `<div class="layout-empty"><strong>${escapeHtml(page.heading || '未命名页面')}</strong><p>当前页面尚未建立 16:9 排版。基础排版使用默认模板，可手工调整。</p><p>智能排版请回到 DSH 会话，让 Agent 按 Pre 方法执行。</p><button class="primary-button" data-layout-ensure type="button">创建手工基础排版</button>${message ? `<small>${escapeHtml(message)}</small>` : ''}</div>`
    return
  }
    ...
    <div class="layout-shell">
      <header class="layout-toolbar">
        ...
        <div class="layout-toolbar-actions">
          ...
          <button data-layout-page-lock type="button">${record.protection?.pageLocked?'解除页面锁定':'锁定此页'}</button>
        ...
        <div class="layout-viewport" data-layout-viewport>
          <div class="layout-canvas-frame" style="width:${width}px;height:${height}px">
            <div class="layout-canvas" data-layout-canvas style="width:${canvas.width}px;height:${canvas.height}px;transform:scale(${zoom})">
              ${renderLayoutElements(record.renderPlan, { assetUrl, selectedId, editable: true })}
            </div>
          </div>
        </div>
        <aside class="layout-inspector">${inspectorHtml(selected)}</aside>
      </div>
      <footer class="layout-status"><span>${escapeHtml(message || (record.stale ? '草案来源已更新，等待同步' : '排版已保存'))}</span><span>${record.renderPlan?.elements?.length ?? 0} 个元素 · 1600 × 900</span></footer>
    </div>`
}
  ...
  try {
    record = await request(`/layout/pages/${encodeURIComponent(page.id)}/mutate`, {
      ...
      body: JSON.stringify({
        ...
        operation,
        ...(lockEditedElement?{lockEditedElement:true,expectedProtectionRevision:record.protection?.revision??0}:{}),
      }),
    })
    loadKey = `${page.id}:${record.state.project.currentRevision}`
    window.reportStudioApplyExternalState?.(clone(record.state))
...
async function toggleProtection(elementId) {
  const page=activePage();if(!page)return
  const locked=elementId?record.protection?.elements?.some(e=>e.layoutElementId===elementId):record.protection?.pageLocked
  try{
    record=await request(`/layout/pages/${encodeURIComponent(page.id)}/protection`,{method:'POST',body:JSON.stringify({baseRevision:hostState.project.currentRevision,expectedProtectionRevision:record.protection?.revision??0,...(elementId?{elementId}:{}),locked:!locked})})
    render(locked?'已解除人工保护':'已锁定；自动任务不会覆盖，仍可手工编辑')
  }catch(error){await loadLayout(true);render(error.message)}
}
...
root.addEventListener('click', event => {
  ...
  if (event.target.closest('[data-layout-page-lock]')) void toggleProtection()
  if (event.target.closest('[data-layout-element-lock]') && selectedId) void toggleProtection(selectedId)
...
root.addEventListener('change', event => {
  if(event.target.matches('[data-layout-lock-on-save]')) {lockEditedElement=event.target.checked;return}
...
window.reportStudioLayoutSync = nextState => {
  ...
  hostState = nextState
  if(record&&Array.isArray(nextState.designProtections))record.protection=clone(nextState.designProtections.find(p=>p.pageId===activePage()?.id)??{pageId:activePage()?.id,revision:0,pageLocked:false,elements:[]})
