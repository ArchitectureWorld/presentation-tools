...
let selectedHistoryRoundId = null;
...
function workspaceStatusView(status = workspaceSyncState) {
  ...
  if (code === 'local_saved_conflict') return { label: '已保存修改与上游更新冲突，当前成果已保留', tone: 'dirty' };
...
function renderWorkspaceStatus(status = workspaceSyncState) {
  ...
  const hasUnignoredDirtyConflict = Boolean(
    ...
    && (workspaceHasDirtyEdits() || status?.status === 'local_saved_conflict')
...
function visualCandidateUrl(proposalId) {
  const prefix = window.location.pathname.startsWith('/report-studio') ? '/report-studio' : '';
  const url = new URL(`${prefix}/api/design/image`, window.location.origin);
...
function renderVisualCandidates(page) {
  const rows = (state.proposals || [])
    .filter(row => row?.kind === 'design.visual.v1' && row.pageId === page.id && !row.linkedAssetId && row.status !== 'accepted')
    .sort((left, right) => String(right.createdAt || '').localeCompare(String(left.createdAt || '')));
  if (!rows.length) return '';
  const labels = { ready: '候选已生成', pending: '候选待确认', approved: '已批准，待挂接', accepted: '已采用并已绑定', link_failed: '挂接失败，可重试' };
  return `<section class="visual-candidate-card" aria-label="本页视觉候选"><header class="asset-card-header"><strong>本页视觉候选</strong><p>${rows.length} 张 · 未采用候选不会进入正式导出</p></header><div class="visual-candidate-grid">${rows.map(row => `<article class="visual-candidate-item"><img src="${escapeAttr(visualCandidateUrl(row.id))}" alt="页面 AI 概念视觉候选"><div><strong>${escapeHtml(labels[row.status] || row.status || '候选')}</strong><small>绑定页面：${escapeHtml(page.heading || page.title || page.id)}</small><small>模型：${escapeHtml(row.provenance?.model || row.provenance?.modelName || row.input?.model || '已授权模型')}</small><p>${escapeHtml(row.input?.prompt || row.provenance?.prompt || '页面概念视觉')}</p></div></article>`).join('')}</div></section>`;
}
...
function renderDraft() {
  ...
  const bullets = bulletItems.map((item, index) => `
    <div class="bullet-row">
      ...
      ${item.listItemId ? `<button class="small-button" data-comment-list-item="${escapeAttr(item.listItemId)}" type="button">批注此要点</button>` : ''}
  ...
  const canonicalAssets = page.assets?.length ? page.assets : (page.pageAssets || []);
  const assets = canonicalAssets.map((asset, index) => renderAsset({ ...asset, id: asset.id || asset.assetId, name: asset.name || asset.displayName, mimeType: asset.mimeType || asset.type }, index)).join('');
    ...
    <div class="stage-shell">
      <header class="stage-header">
        <div class="stage-heading">
          ...
          <h1>草案阶段</h1>
          <p>正文段落和正文要点都会进入页面展示；讲解稿仅用于讲述。选中文字后点击对应批注按钮，可只修改选中的内容。</p>
        </div>
        <div class="stage-header-actions">
        ...
        <div class="draft-grid">
          <section class="draft-card">
            ...
            <div class="field-block">
              <div class="field-heading"><label for="draft-heading">页面标题</label><span>本页核心结论 <button class="small-button" data-comment-draft-field="title" type="button">批注标题</button></span></div>
              <input id="draft-heading" class="draft-input" value="${escapeAttr(buffer.heading)}">
            </div>

            <div class="field-block">
              <div class="field-heading"><label for="draft-body">正文段落</label><span>连续文字 <button class="small-button" data-comment-draft-field="body" type="button">批注正文 / 选中文字</button></span></div>
              <textarea id="draft-body" class="draft-textarea" rows="8">${escapeHtml(buffer.body)}</textarea>
            </div>

            <div class="field-block">
              <div class="field-heading"><label>正文要点</label><span>分条呈现的页面正文，支持逐条编辑和批注</span></div>
...
function submissionHtml(submission) {
  const labels = {
    pending_dispatch: '等待投递', dispatched: '已投递', dispatch_failed: '投递失败', apply_failed: '修改失败，可重试', conflict: '版本冲突，可重试', no_changes: '未执行修改',
    proposal_created: '处理中', accepted: '已应用', stale: '已过期',
    rejected: '已结束', returned_to_agent: '已重新投递',
  };
  return `
    <div class="submission-card">
      ...
      <div>
        ...
        <span>baseRevision ${submission.baseRevision} · ${escapeHtml(submission.completionStatus === 'partially_completed' ? '部分完成' : labels[submission.status] || submission.status)}</span>
        ${submission.lastDispatchError ? `<small class="submission-error">${escapeHtml(submission.lastDispatchError)}</small>` : ''}
        ${!submission.supersededBy && ['pending_dispatch', 'dispatch_failed', 'apply_failed', 'conflict'].includes(submission.status) ? `<button class="small-button" data-retry-submission="${escapeAttr(submission.id)}" type="button">继续投递</button>` : ''}
      </div>
    </div>
  `;
}
...
function proposalHtml(proposal) {
  ...
  const reviewRun = (state.reviewRuns ?? []).find(run => run.resultProposalId === proposal.id || run.reviewSubmissionId === proposal.submissionId);
  const nativeAutoApply = Boolean(window.reportStudioNativeCapabilities?.ordinaryProposalAutoApply);
  const autoApplyInProgress = proposal.status === 'pending'
    && proposal.aggregateRiskLevel === 'ordinary_reversible'
    && nativeAutoApply
    && !stale
    && reviewRun
    && !reviewRun.closedAt
    && ['queued', 'reading_context', 'processing', 'proposal_created'].includes(reviewRun.phase)
    && reviewRun.integrationState !== 'dispatch_failed';
  const pendingActions = proposal.status === 'pending'
    ? `<div class="proposal-actions">${autoApplyInProgress
      ? '<span class="proposal-auto-apply-status">自动应用中…</span>'
      : `<button class="primary-button batch-submit" data-accept-proposal="${escapeAttr(proposal.id)}" type="button" ${stale ? 'disabled' : ''}>${stale ? 'Proposal 已过期' : '确认应用'}</button>`}<button class="small-button danger" data-reject-proposal="${escapeAttr(proposal.id)}" type="button">拒绝</button><button class="small-button" data-return-proposal="${escapeAttr(proposal.id)}" type="button">返回 Agent 调整</button></div>`
    : '';
  return `
...
function submissionVisibleForFilter(submission, proposal) {
  ...
  const unfinished = submission.completionStatus === 'partially_completed' || ['pending_dispatch', 'dispatched', 'dispatch_failed', 'apply_failed', 'conflict', 'no_changes', 'proposal_created', 'stale'].includes(submission.status);
  return commentFilter === 'unfinished' ? unfinished : submission.status === 'accepted' && submission.completionStatus !== 'partially_completed';
}
...
function roundHasLiveWork(round, annotations, submissions, proposals, runs) {
  ...
  const roundProposals = proposals.filter(proposal => proposal.kind && proposal.reviewRoundId === round.id);
  const roundRuns = runs.filter(run => roundSubmissions.some(submission => submission.id === run.reviewSubmissionId));
  return roundAnnotations.some(annotation => annotation.lifecycle === 'draft')
    || roundSubmissions.some(submission => ['pending_dispatch', 'dispatched', 'dispatch_failed', 'proposal_created'].includes(submission.status))
    || roundProposals.some(proposal => proposal.status === 'pending')
    || roundRuns.some(run => !run.closedAt && !['completed', 'failed', 'timed_out'].includes(run.phase));
}

function reviewRoundHtml(round, annotations, submissions, proposals, { current = false, draftAnnotations = [] } = {}) {
  const visibleRoundAnnotations = filteredAnnotations(annotations.filter(annotation => annotation.reviewRoundId === round.id));
  const proposalsBySubmission = new Map(proposals.filter(proposal => proposal.kind).map(proposal => [proposal.submissionId, proposal]));
  ...
  const orphanProposals = proposals.filter(proposal => proposal.kind)
    .filter(proposal => proposal.reviewRoundId === round.id && !submissions.some(submission => submission.id === proposal.submissionId) && proposalVisibleForFilter(proposal));
  const roundAnnotations = annotations.filter(annotation => annotation.reviewRoundId === round.id);
  ...
  if (!visibleRoundAnnotations.length && !visibleSubmissionGroups.length && !orphanProposals.length && commentFilter !== 'all') return '';
  return `
    <section class="comment-batch ${current ? 'comment-batch-current' : ''}" data-current-round="${current ? escapeAttr(round.id) : ''}">
      <header class="comment-batch-header">
        <div class="batch-heading">
          <strong>${current ? '当前轮次' : `批注轮次 · ${escapeHtml(round.id.slice(-5))}`}</strong>
          <small>已完成 ${stats.completed} · 未完成 ${stats.unfinished} · 已提交 ${submissions.filter(submission => submission.reviewRoundId === round.id).length} 次</small>
        </div>
        <div class="batch-actions">
          ...
          ${pendingDrafts.length
            ...
            : !current ? `<button class="small-button" data-continue-round="${escapeAttr(round.id)}" type="button">继续本轮</button>` : ''}
        </div>
      </header>
      <div class="comment-batch-body">
        ${draftAnnotations.length ? `<section class="comment-draft-group"><strong>本轮未提交</strong>${draftAnnotations.map(annotationHtml).join('')}</section>` : ''}
        ${visibleRoundAnnotations.map(annotationHtml).join('') || (current ? '<div class="empty-comments">当前轮次还没有已提交批注。</div>' : '<div class="empty-comments">当前筛选下没有批注。</div>')}
...
function renderAnnotations() {
  ...
  if (selectedHistoryRoundId && !scopeRounds.some(round => round.id === selectedHistoryRoundId)) selectedHistoryRoundId = null;
  ...
  const pendingProposals = state.proposals.filter(proposal => proposal.kind && scopeRoundIds.has(proposal.reviewRoundId) && proposal.status === 'pending');
  ...
  const currentRound = scopeRounds.slice().reverse().find(round => roundHasLiveWork(round, annotations, state.reviewSubmissions, state.proposals, state.reviewRuns ?? []));
  const displayedRound = selectedHistoryRoundId
    ? scopeRounds.find(round => round.id === selectedHistoryRoundId) ?? currentRound
    : currentRound;
  const historyRounds = scopeRounds.slice().reverse().filter(round => round.id !== currentRound?.id);
  const html = [`
    <div class="review-history-shell">
      <nav class="review-history-rail" aria-label="历史轮次">
        <button class="history-round-button ${!selectedHistoryRoundId ? 'active' : ''}" data-current-round="true" type="button">当前</button>
        ${historyRounds.map(round => `<button class="history-round-button ${selectedHistoryRoundId === round.id ? 'active' : ''}" data-history-round="${escapeAttr(round.id)}" type="button">${escapeHtml(round.id.slice(-4))}</button>`).join('')}
      </nav>
  ...
  if (!displayedRound && !selectedHistoryRoundId) {
    ...
    `);
      ...
      html.push('<section class="comment-batch comment-batch-current" data-current-empty="true"><div class="empty-comments">当前轮次暂无批注，添加一条批注后开始。</div></section>');
    }
  } else if (displayedRound) {
    html.push(reviewRoundHtml(displayedRound, annotations, state.reviewSubmissions, state.proposals, {
      current: displayedRound.id === currentRound?.id && !selectedHistoryRoundId,
      draftAnnotations: displayedRound.id === currentRound?.id && !selectedHistoryRoundId ? currentVisible : [],
    }));
  }
  html.push('</div></div>');
  query('#review-history').innerHTML = html.join('');
...
function renderAgent() {
  ...
  const reviewStatus = status => ({
    ...
    proposal_created: '历史提案',
    accepted: '修改已保存',
    rejected: '已结束',
    stale: '已过期',
    apply_failed: '修改失败，可重试',
    conflict: '版本冲突，可重新规划',
    no_changes: '未执行修改',
  })[status] || status || '状态未知';
  ...
  for (const run of state.reviewRuns ?? []) {
    const previous = latestRun.get(run.reviewSubmissionId);
    if (!previous || Number(run.dispatchAttempt || 0) >= Number(previous.dispatchAttempt || 0)) latestRun.set(run.reviewSubmissionId, run);
  }
  const items = state.reviewSubmissions.slice(-12).map(submission => {
    const run = latestRun.get(submission.id);
    const failed = ['dispatch_failed', 'apply_failed', 'conflict'].includes(submission.status);
    const status = failed ? reviewStatus(submission.status) : run ? taskPhase(run.phase) : reviewStatus(submission.status);
    const summary = run?.summary || submission.agentMessage || '';
    const error = run?.lastError || submission.lastDispatchError || '';
    const detail = error && error !== summary ? `<br>${escapeHtml(error)}` : '';
    const canRetry = failed && !submission.supersededBy;
    const stamp = new Date(submission.createdAt);
    const retryLabel = Number.isNaN(stamp.getTime()) ? submission.id : stamp.toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    const retryButton = canRetry ? `<button class="small-button" type="button" data-retry-submission="${escapeAttr(submission.id)}">重试批注 · ${escapeHtml(retryLabel)}</button>` : '';
    const executor = run?.executionMode === 'native' ? `<span class="agent-message-meta">DSH 主会话${run.executionModel?.model ? ` · ${escapeHtml(run.executionModel.provider || '')} / ${escapeHtml(run.executionModel.model)}` : ' · 等待实际模型记录'}</span>` : '';
    return {
      time: run?.createdAt || submission.createdAt,
      html: `<div class="agent-message ${failed ? 'error' : 'system'}"><strong>批注任务 · 第 ${Number(submission.number || 1)} 次投递</strong><br>${escapeHtml(status)}${summary ? `<br>${escapeHtml(summary)}` : ''}${detail}${executor}<span class="agent-message-meta">${canRetry ? '可重试' : submission.supersededBy ? '已由后续任务继续处理' : submission.completionStatus === 'partially_completed' ? '部分完成' : submission.status === 'no_changes' ? '未完成' : run?.closedAt ? '已完成' : '处理中'}</span>${retryButton}</div>`,
    };
  }).sort((a, b) => String(a.time || '').localeCompare(String(b.time || '')));

  const welcome = `
    <div class="agent-welcome">
      <strong>批注处理任务</strong>
      <p>${escapeHtml(state.project.title)}</p>
    </div>`;
...
function render() {
  ...
  window.reportStudioDesignSync?.(structuredClone(state));
}
...
function reviewTaskStateIsTerminal(nextState, { submissionId = null, reviewRunId = null, autoApplyOrdinary = false } = {}) {
  const reviewRun = reviewRunId
    ? (nextState.reviewRuns ?? []).find(run => run.reviewRunId === reviewRunId)
    : (nextState.reviewRuns ?? []).find(run => run.reviewSubmissionId === submissionId);
  const submission = (nextState.reviewSubmissions ?? []).find(item => item.id === (submissionId ?? reviewRun?.reviewSubmissionId));
  if (['dispatch_failed', 'apply_failed', 'conflict', 'no_changes'].includes(submission?.status)) return true;
  if (['completed', 'failed', 'timed_out'].includes(reviewRun?.phase)) return true;
  if (submission?.status === 'accepted' || reviewRun?.integrationState === 'accepted') return true;
  return false;
}

window.reportStudioApplyTaskState = (nextState, options = {}) => {
  if (!nextState || !state) return false;
  const terminal = reviewTaskStateIsTerminal(nextState, options);
  const nextRevision = Number(nextState.project?.currentRevision);
  const currentRevision = Number(state.project?.currentRevision);
  const canApplyFullState = terminal
    && nextState.project
    && Array.isArray(nextState.pages)
    && Number.isSafeInteger(nextRevision)
    && Number.isSafeInteger(currentRevision)
    && nextRevision >= currentRevision;
  if (canApplyFullState) {
    state = nextState;
    render();
    return true;
  }
  for (const key of ['reviewRuns', 'reviewSubmissions', 'reviewRounds', 'proposals']) {
    if (Array.isArray(nextState[key])) state[key] = nextState[key];
  }
  renderAgent();
  renderAnnotations();
  return terminal;
...
async function submitReview(reviewRoundId = selectedRoundId) {
  ...
  try {
    ...
    if (result.bridgeResult?.message) toast(result.bridgeResult.message);
...
function openAgent() {
  ...
  renderAgent();
  window.requestAnimationFrame(() => (query('#agent-input')?.disabled ? query('#agent-close') : query('#agent-input'))?.focus());
}
...
document.addEventListener('click', async event => {
  const confirmTask = event.target.closest('[data-confirm-review-task]');
  if (confirmTask) {
    try {
      const result = await api(`/api/review-task/${encodeURIComponent(confirmTask.dataset.confirmReviewTask)}/confirm`, { method: 'POST' });
      window.reportStudioApplyTaskState(result.state);
      toast('本轮批注任务已确认完成');
  ...
  const historyRound = event.target.closest('[data-history-round]');
  if (historyRound) {
    selectedHistoryRoundId = historyRound.dataset.historyRound;
    selectedRoundId = null;
    renderAnnotations();
    return;
  }

  const currentRound = event.target.closest('[data-current-round]');
  if (currentRound && currentRound.dataset.currentRound === 'true') {
    selectedHistoryRoundId = null;
    selectedRoundId = null;
    renderAnnotations();
    return;
  ...
  const commentField = event.target.closest('[data-comment-draft-field]');
  const commentItem = event.target.closest('[data-comment-list-item]');
  if (commentField || commentItem) {
    const isBody = commentField?.dataset.commentDraftField === 'body';
    const itemId = commentItem?.dataset.commentListItem;
    const input = itemId ? query(`[data-list-item-id="${itemId}"]`) : query(isBody ? '#draft-body' : '#draft-heading');
    const start = input?.selectionStart; const end = input?.selectionEnd;
    const selection = (isBody || itemId) && Number.isInteger(start) && Number.isInteger(end) && end > start
      ? { start, end, quote: String(input.value).slice(start, end) } : null;
    if (!await flushDraftBuffer({ reason: '定位批注对象' })) return;
    const page = activePage();
    if (!page) return;
    const block = itemId
      ? page.contentBlocks.find(value => value.type === 'list' && value.items.some(item => item.listItemId === itemId))
      : page.contentBlocks.find(value => value.contentBlockId === (isBody ? page.contentBlocks.find(value => value.type === 'text' && value.role === 'body')?.contentBlockId : page.titleBlockId));
    if (!block) { toast('请先保存正文内容，再定位批注。', true); return; }
    const label = itemId ? `正文要点：${block.items.find(item => item.listItemId === itemId).content.slice(0, 50)}` : isBody ? '正文段落' : '页面标题';
    selectedTarget = { type: itemId ? 'draft-list-item' : isBody ? 'draft-text' : 'draft-title', id: itemId || block.contentBlockId, pageId: page.id, blockId: block.contentBlockId,
      label: selection ? `${label} · “${selection.quote.slice(0, 60)}”` : label, ...(selection ? { selection } : {}) };
    renderAnnotations();
