import {executeProductionApi,readDeliveryFile} from './production-api.mjs';
import {assertDesignHostRequest} from './host-request.mjs';
import {createDeliveryExportService} from './delivery-export.mjs';
...
export async function createStudioServer({ dataDir = process.env.REPORT_STUDIO_DATA_DIR || join(process.cwd(), '.report-studio-data'), port = Number(process.env.PORT || 4173), host = process.env.HOST || '127.0.0.1', agentBridge = undefined } = {}) {
  ...
  const deliveryService = createDeliveryExportService({repository,layoutService});
  ...
  async function handleApi(req, res, url) {
    if (url.pathname === '/api/production/export') return sendJson(res,200,await executeProductionApi({runtime:{deliveryFor:async()=>deliveryService},sessionId:bridgeSessionId,operation:'export',request:req,body:await readJson(req)}));
    if (req.method==='GET'&&url.pathname==='/api/delivery/file'){
      const result=await readDeliveryFile({repository,deliveryId:url.searchParams.get('deliveryId'),name:url.searchParams.get('name')});res.writeHead(200,{'content-type':result.mimeType,'content-disposition':`attachment; filename="${result.name}"`,'content-length':result.bytes.length,'cache-control':'no-store','x-content-type-options':'nosniff'});res.end(result.bytes);return;
    }
    ...
    if (layoutMatch) {
      try {
        if(req.method==='POST'&&layoutMatch.operation==='protection')assertDesignHostRequest(req);
  ...
  server = http.createServer(async (req, res) => {
    ...
    } catch (error) { sendJson(res, error.statusCode || error.status || 400, error?.code ? errorPayload(error) : { error: error.message || 'request_failed' }); }
  });
