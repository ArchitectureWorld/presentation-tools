...
test('browser app contains production actions for outline, draft, review and proposal', async () => {
  ...
  for (const token of ['outline.add','outline.rename','draft.ensurePage','draft.update','annotation.add','/api/review/submit','/accept']) assert.ok(app.includes(token), `missing ${token}`);
