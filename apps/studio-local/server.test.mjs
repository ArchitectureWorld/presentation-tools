...
test('review submission through configured bridge directly applies the change', async () => {
  ...
  try {
    ...
    assert.equal(review.bridgeResult.message, '建议修改标题'); assert.equal(review.bridgeResult.proposalId, null); assert.equal(review.state.proposals.length, 0); assert.equal(review.state.outline[0].title, '第一章：目标');
    assert.equal(review.state.reviewSubmissions[0].status, 'accepted');
    ...
    assert.equal(review.state.reviewRuns[0].integrationState, 'accepted');
    assert.equal(review.state.reviewRuns[0].resultProposalId, null);
...
test('configured bridge failure is persisted and the same submission can be retried', async () => {
  ...
  try {
    ...
    assert.equal(retriedPayload.state.reviewSubmissions[0].status, 'no_changes');
    assert.equal(retriedPayload.state.annotations[0].resolution, 'open');
    assert.deepEqual(retriedPayload.state.reviewRuns.map(run => run.dispatchAttempt), [1, 2]);
    assert.deepEqual(retriedPayload.state.reviewRuns.map(run => run.integrationState), ['dispatch_failed', 'no_changes']);
  } finally { await app.stop(); await rm(dir, { recursive:true, force:true }); }
});
...
test('pending Submission survives server restart and resumes without replacement', async () => {
  ...
  try {
    ...
    assert.equal(resumed.state.reviewSubmissions[0].status, 'no_changes');
    assert.equal(resumed.state.annotations[0].resolution, 'open');
