...
export function matchLayoutApiPath(pathname, prefix = '/api') {
  ...
  const match = new RegExp(`^${escaped}/layout/pages/([^/]+)(?:/(ensure|mutate|reconcile|protection))?$`, 'u').exec(pathname)
  ...
  switch (match.operation) {
    ...
    case 'reconcile':
      ...
      return service.ensure({
        pageId: match.pageId,
        baseRevision: integer(body.baseRevision, 'baseRevision'),
        source: 'layout-reconcile',
      })
    case 'protection':
      if (verb !== 'POST') fail('layout_api_method_not_allowed', '修改人工保护仅支持 POST。', undefined, 405)
      if (Object.keys(body).some(key => !['baseRevision','expectedProtectionRevision','elementId','locked'].includes(key)) || typeof body.locked !== 'boolean') fail('protection_invalid_input','保护设置包含未知字段或无效值。')
      return service.updateProtection({pageId:match.pageId,baseRevision:integer(body.baseRevision,'baseRevision'),expectedProtectionRevision:integer(body.expectedProtectionRevision,'expectedProtectionRevision'),...(body.elementId!==undefined?{elementId:body.elementId}:{}),locked:body.locked})
    case 'mutate':
      ...
      return service.mutate({
        ...
        ...(body.lockEditedElement !== undefined ? {lockEditedElement:body.lockEditedElement === true,expectedProtectionRevision:integer(body.expectedProtectionRevision,'expectedProtectionRevision')} : {}),
...
export function layoutApiErrorPayload(error) {
  const known = /^(?:protection_|design_protected$)/u.test(error?.code ?? '') || error instanceof LayoutApiError || error instanceof LayoutServiceError || error instanceof LayoutPersistenceError
  return {
