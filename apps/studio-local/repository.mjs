import { assertProtectedContent } from './design-protection.mjs'
...
function operationalFromState(state, revisions = state.revisions ?? []) {
  const designMetadata = { designRuns: state.designRuns ?? [], layoutCandidates: state.layoutCandidates ?? [], designProtections: state.designProtections ?? [], designBatches: state.designBatches ?? [] }
  const seen = new Set()
  const invalid = () => { throw new StudioError(ERROR_CODES.INVALID_COMMAND, '设计运行态只接受有限 JSON 元数据和 ObjectStore 引用。') }
  function check(value) {
    if (value === null || typeof value === 'boolean' || typeof value === 'string') return
    if (typeof value === 'number' && Number.isFinite(value)) return
    if (!value || typeof value !== 'object' || seen.has(value) || (!Array.isArray(value) && Object.getPrototypeOf(value) !== Object.prototype)) invalid()
    seen.add(value)
    for (const [key, child] of Object.entries(value)) {
      if (/^(?:png|dataUrl|dataBase64|base64|bytes)$/iu.test(key)) invalid()
      check(child)
    }
    seen.delete(value)
  }
  if (!Object.values(designMetadata).every(Array.isArray)) invalid()
  check(designMetadata)
  if (Buffer.byteLength(JSON.stringify(designMetadata)) > 2_000_000) invalid()
  return {
    ...
    designRuns: clone(state.designRuns ?? []),
    layoutCandidates: clone(state.layoutCandidates ?? []),
    designProtections: clone(state.designProtections ?? []),
    designBatches: clone(state.designBatches ?? []),
...
function isUnusedWorkspace(state, currentControl) {
  ...
  const collections = [
    ...
    state.proposals,
    state.reviewRuns,
    state.designRuns, state.layoutCandidates, state.designProtections, state.designBatches,
...
export async function createRepository(dataDir, { faultInjector = () => undefined, fileOpen = open } = {}) {
  ...
  async function transactContent(input, mutator) {
    ...
    return enqueue(async () => {
      ...
      assertProtectedContent(baseState, candidate, {human:['human','human-layout'].includes(input.source ?? 'human')})
  ...
  async function publishUpstreamSnapshot({ snapshot, fingerprint, workspaceRoot, sourceRevision = null, sourceRevisions = [], discardLocalChanges = false }) {
    assertReady()
    ...
    return transactContent(
      ...
      async state => {
        // Compare saved content, not just the browser's unsaved-input flag.
        // Run inside the content transaction so a concurrent save cannot slip past.
        if (snapshot.project.id !== state.project.id) {
          throw new StudioError(ERROR_CODES.INVALID_REFERENCE, '上游项目身份与当前工作区不一致。', undefined, 409)
        }
        const baseline = [...state.revisions].reverse().find(row => row.detail?.actionType === 'workspace.upstream_publish')
          ?? state.revisions.find(row => row.source === 'standard_import')
        const imported = baseline ? await getSnapshotAt(baseline.number) : null
        if (discardLocalChanges !== true && (!imported || canonicalJson(canonicalFromState(state)) !== canonicalJson(imported))) {
          throw new StudioError(ERROR_CODES.WORKSPACE_SAVED_CONFLICT,
            '当前汇报包含已保存修改，已保留本地成果。请核对差异后决定是否替换。',
            { baseRevision: baseline?.number ?? null, currentRevision: state.project.currentRevision, fingerprint }, 409)
        }
        return projectStateFromParts({ snapshot, currentRevision: state.project.currentRevision,
          operational: operationalFromState(state, state.revisions), ui })
      },
    )
  }
  ...
  return {
    ...
    applyMigration,
    getSnapshotAt,
    // Internal host services only. Candidate bytes never enter Canonical.
    async putLayoutCandidate(value) {
      assertReady()
      const payload = canonicalJson(value)
      if (Buffer.byteLength(payload) > 2_000_000) throw new StudioError(ERROR_CODES.INVALID_COMMAND, '布局候选超过大小限制。')
      return putObject({ kind: 'LayoutDesignCandidate', value })
    },
    async getLayoutCandidate(ref) {
      assertReady()
      if (!/^[a-f0-9]{64}$/u.test(String(ref?.sha256 ?? ''))) throw new StudioError(ERROR_CODES.INVALID_REFERENCE, '候选对象引用无效。')
      const stored = await getObject(ref)
      if (stored.kind !== 'LayoutDesignCandidate') throw new StudioError(ERROR_CODES.INVALID_REFERENCE, '对象不是布局候选。')
      return clone(stored.value)
    },
    putBlob,
    openBlob,
