...
test('direct agent apply commits every allowed review command without creating a Proposal', () => {
  const submitted = delivered(outlineSubmission())
  const node = submitted.state.outline[0]
  const command = {
  ...
  const beforeRevision = submitted.state.project.currentRevision
  const result = applyCommandsFromAgent(submitted.state, submitted.submission.id, envelope(submitted.state, submitted.submission, [command]))
  assert.equal(result.state.outline[0].title, '直接应用后的标题')
  assert.equal(result.state.project.currentRevision, beforeRevision + 1)
  assert.equal(result.state.proposals.length, 0)
  assert.equal(result.state.reviewSubmissions[0].status, 'accepted')
  assert.equal(result.state.annotations[0].resolution, 'resolved')
  assert.equal(result.state.reviewRuns[0].integrationState, 'accepted')
  assert.equal(result.state.reviewRuns[0].phase, 'completed')
  assert.equal(result.state.reviewRuns[0].resultProposalId, null)
  assert.equal(result.state.revisions.at(-1).detail.summary, 'Agent 修改建议')
})
