export { designContextResource } from './design-context.mjs'
...
function annotationTargetProjection(target) {
  ...
  return {
    ...
    ...(safeString(target.pageId) !== undefined ? { pageId: safeString(target.pageId) } : {}),
    ...(target.blockId === null || safeString(target.blockId) !== undefined ? { blockId: target.blockId === null ? null : safeString(target.blockId) } : {}),
    ...(target.selection && typeof target.selection === 'object' ? { selection: {
      start: Number.isInteger(target.selection.start) ? target.selection.start : undefined,
      end: Number.isInteger(target.selection.end) ? target.selection.end : undefined,
      quote: safeString(target.selection.quote),
    } } : {}),
  }
}
