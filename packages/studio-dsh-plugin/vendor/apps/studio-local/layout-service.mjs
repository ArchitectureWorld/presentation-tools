import {pageProjectDependencies} from './source-dependencies.mjs'
import { applyProtectionUpdate, protectionFor, assertProtectedLayout, createDesignProtectionService } from './design-protection.mjs'
...
import { validateLayoutTypography, DESIGN_RENDER_CAPABILITIES } from '../../packages/studio-layout-core/design-validation.mjs'
...
function projectionForState(state, pageId) {
  ...
  // Layout publication must not invalidate its own sources. Canonical rules,
  // draft/scripts and imported material object hashes do participate.
  const { layoutRef, ...sourcePage } = snapshot.pages.find(item => item.id === page.id)
  return { ...first, sourceStateHash: sourceHash({ sources: first.sources, project: pageProjectDependencies(snapshot.project,sourcePage), page: sourcePage }) }
}
...
function fitBodyEntries(entries, startY, imageCount) {
  ...
  for (const compact of [false, true]) {
    ...
    for (let fontSize = 28; fontSize >= DESIGN_RENDER_CAPABILITIES.minimumBodyFontSize; fontSize -= 1) {
      let y = startY
...
function defaultLayout(projection) {
  ...
  let bodyY = 198
  if (title) {
    const fitted = fitTextEntries([title], 1416, 128, 48, DESIGN_RENDER_CAPABILITIES.minimumTitleFontSize)
...
function responseFor(state, layout, ref, projection, renderPlan, extra = {}) {
  return {
    state,
    protection: protectionFor(state,projection.pageId),
    layout,
    layoutRef: ref,
    sourceProjection: {
      projectId: projection.projectId,
...
export function createLayoutService({ repository, layoutRoot = join(repository.root, 'layouts'), faultInjector = () => undefined } = {}) {
  ...
  async function loadExisting(state, page, projection, { repair = true } = {}) {
    ...
    if (pageRef) {
      ...
      if (repair && currentRef?.sha256 !== pageRef.sha256) await store.repairRef(pageRef)
    } else {
  ...
  async function attachPrepared({ before, pageId, prepared, source, updateOperational }) {
    if (prepared.noOp) {
      const page = pageForState(before, pageId)
      if (page.layoutRef?.sha256 === prepared.ref.sha256) {
        const state = updateOperational ? await repository.transactOperational(draft => {
          if (draft.project.currentRevision !== before.project.currentRevision || pageForState(draft, pageId).layoutRef?.sha256 !== prepared.ref.sha256) fail('design_stale_baseline', '设计基线已变化。', undefined, 409)
          updateOperational(draft)
          return draft
        }) : before
        return { state, record: prepared, projectNoOp: true }
      }
    }
    ...
    await faultInjector('after_layout_head_publish', { pageId, prepared })
  ...
  async function mutate({ pageId, baseRevision, expectedLayoutRevision, operation, source = 'human', lockEditedElement = false, expectedProtectionRevision } = {}) {
    const before = repository.getState()
    if (before.project.currentRevision !== baseRevision) {
    ...
    if (!['human','human-layout'].includes(source)) assertProtectedLayout(before,pageId,existing.layout,candidate)
    assertLayoutPageDocument(candidate)
    // Legacy pages may contain several undersized texts. Validate the edited
    // element so users can repair them one by one; Agent candidates validate
    // the whole page before preview/submission in validateDesignLayout.
    const editedLayout = { ...candidate, elements: candidate.elements.filter(element => element.layoutElementId === operation.layoutElementId) }
    const validation = validateLayoutTypography({ layout: editedLayout, sources: projection.sources })
    if (!validation.valid) fail('design_validation_failed', '字号低于可读下限：1600 × 900 画布上标题至少 36px、正文 24px、图注或脚注 16px；请放大文本或拆分页面。', validation, 400)
    const prepared = await store.preparePage(candidate, {
      expectedLayoutRevision,
      sourceProjectRevision: projection.projectRevision,
      sourceStateHash: projection.sourceStateHash,
    })
    const updateOperational = draft => {
      if (!['human','human-layout'].includes(source)) assertProtectedLayout(draft,pageId,existing.layout,candidate)
      if (lockEditedElement) {
        if (!['human','human-layout'].includes(source)) fail('design_protected','只有人工可以设置保护。',undefined,403)
        applyProtectionUpdate(draft,{pageId,baseRevision,expectedProtectionRevision,elementId:operation.layoutElementId,locked:true},prepared.layout)
      }
    }
    const attached = await attachPrepared({ before, pageId, prepared, source, updateOperational })
    const currentProjection = projectionForState(attached.state, pageId)
  ...
  // Narrow host-only integration points for frozen design candidates. These
  // are not editable layout operations and must not be exposed as Agent tools.
  async function designContext({ pageId }) {
    const state = repository.getState()
    const page = pageForState(state, pageId)
    const projection = projectionForState(state, pageId)
    const existing = await loadExisting(state, page, projection, { repair: false })
    return { state, projection, pageAssets: resolvedPageAssets(page), layout: existing?.layout ?? null, layoutRef: existing?.ref ?? null }
  }
  async function prepareDesign({ pageId, layout, baseProjectRevision, baseLayoutRevision, baseLayoutSha, sourceStateHash }) {
    const ctx = await designContext({ pageId })
    if (ctx.state.project.currentRevision !== baseProjectRevision || (ctx.layoutRef?.layoutRevision ?? null) !== baseLayoutRevision
      || (ctx.layoutRef?.sha256 ?? null) !== baseLayoutSha || ctx.projection.sourceStateHash !== sourceStateHash) fail('design_stale_baseline', '设计基线已变化。', undefined, 409)
    // Recover an authoritative Canonical pointer before Store CAS if necessary.
    if (ctx.layoutRef && (await store.readRef(pageId))?.sha256 !== ctx.layoutRef.sha256) await store.repairRef(ctx.layoutRef)
    const prepared = await store.preparePage(layout, { expectedLayoutRevision: baseLayoutRevision ?? -1, sourceProjectRevision: baseProjectRevision, sourceStateHash })
    return { prepared, renderPlan: createLayoutRenderPlan(prepared.layout, ctx.projection.sources), pageAssets: ctx.pageAssets }
  }
  async function publishDesign({ pageId, prepared, baseProjectRevision, baseLayoutRevision, baseLayoutSha, sourceStateHash, updateOperational }) {
    const ctx = await designContext({ pageId })
    if (ctx.state.project.currentRevision !== baseProjectRevision || (ctx.layoutRef?.layoutRevision ?? null) !== baseLayoutRevision
      || (ctx.layoutRef?.sha256 ?? null) !== baseLayoutSha || ctx.projection.sourceStateHash !== sourceStateHash) fail('design_stale_baseline', '设计基线已变化。', undefined, 409)
    const record = await store.readByRef(prepared.ref)
    if (JSON.stringify(canonicalize(record.layout)) !== JSON.stringify(canonicalize(prepared.layout))) fail('design_candidate_mismatch', '布局候选内容不匹配。', undefined, 409)
    return attachPrepared({ before: ctx.state, pageId, prepared, source: 'host-design', updateOperational })
  }
  const protections = createDesignProtectionService({repository,layoutService:{designContext}})
  async function updateProtection(input) {
    await protections.update(input)
    return {...await get({pageId:input.pageId,reconcile:false}),protection:protections.get({pageId:input.pageId})}
  }
  return Object.freeze({ root: store.root, store, ensure, get, mutate, designContext, prepareDesign, publishDesign, updateProtection })
}
