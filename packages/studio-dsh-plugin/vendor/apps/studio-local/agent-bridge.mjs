function validateResult(value) {
  ...
  return {
    ...
    ...(value.annotationResults ? { annotationResults: structuredClone(value.annotationResults) } : {}),
...
export function createAgentBridge({ url = process.env.REPORT_STUDIO_AGENT_URL || '', fetchImpl = fetch, timeoutMs = Number(process.env.REPORT_STUDIO_AGENT_TIMEOUT_MS || 60000) } = {}) {
  ...
  async function call(payload, signal) {
    ...
    try {
      const response = await fetchImpl(endpoint, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload), signal: signal ? AbortSignal.any([signal, controller.signal]) : controller.signal });
  ...
  return {
    ...
    submit({ submission, context = null, taskId = null, parentSessionId = null, leaseMs = null, signal }) {
      return call({ kind: 'report_studio.review_submission', executionMode: 'isolated', taskId, parentSessionId, leaseMs, submission, context }, signal);
    },
    async close({ taskId, workerSessionRef = null, parentSessionId = null }) {
      if (!endpoint) throw new Error('DSH Bridge 未配置');
      const response = await fetchImpl(endpoint, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ kind: 'report_studio.review_task.close', executionMode: 'isolated', taskId, workerSessionRef, parentSessionId }),
