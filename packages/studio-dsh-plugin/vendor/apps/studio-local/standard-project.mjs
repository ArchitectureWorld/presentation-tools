import { captureCheckedDelivery } from './delivery-snapshot.mjs'
...
/** Bootstrap interchange may omit LayoutService; finished exports always supply it. */
async function assertLayoutExportReady(repository, layoutService) {
  if (!layoutService || !repository.getState().pages.length) return
  return captureCheckedDelivery({repository,layoutService})
}
...
export function createStandardProjectService(repository, options = {}) {
  ...
  return Object.freeze({
    ...
    async exportProject() {
      const state = repository.getState()
      const checked = await assertLayoutExportReady(repository, layoutService)
      if (checked && checked.revision !== state.project.currentRevision) throw new StudioError('delivery_stale_revision', '导出来源版本已变化。', undefined, 409)
      ...
      try {
        ...
        if (repository.getState().project.currentRevision !== state.project.currentRevision) throw new StudioError('delivery_stale_revision', '标准项目在导出期间变化。', undefined, 409)
