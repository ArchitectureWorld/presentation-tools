...
// Resolve only registered, project-local ObjectRefs. A manifest record is authoritative
// over a legacy page asset; inconsistent metadata never falls back to another file.
export function projectAssetCatalog(state) {
  const archive = state.project.extensionPayload?.standardArchive
  const records = archive?.documents?.['assets/manifest.json']?.assets ?? []
  const linked = state.pages.flatMap(page => [...(page.assets ?? []), ...(page.pageAssets ?? [])])
  const catalog = new Map()
  const matches = (value, ref) => ref && /^[a-f0-9]{64}$/i.test(ref.sha256 ?? '') && Number.isSafeInteger(ref.sizeBytes) && ref.sizeBytes >= 0 && typeof ref.mimeType === 'string' && ['sha256', 'sizeBytes', 'mimeType'].every(key => value[key] === ref[key])
  for (const record of records) {
    const files = (archive.files ?? []).filter(file => file.relativePath === record.relativePath)
    const file = files[0]
    const ref = file ? file.objectRef : linked.find(asset => (asset.assetId ?? asset.id) === record.assetId)?.objectRef
    const unique = records.filter(row => row.assetId === record.assetId).length === 1 && files.length <= 1
    const valid = unique && matches(record, ref) && (!file || ['sha256', 'sizeBytes', 'mimeType'].every(key => file[key] === undefined || file[key] === ref?.[key]))
    catalog.set(record.assetId, { id: record.assetId, name: record.displayName, mimeType: record.mimeType, sizeBytes: record.sizeBytes, sha256: record.sha256, ...record.metadata, sourceRefs: structuredClone(record.sourceRefs ?? []), objectRef: valid ? structuredClone(ref) : null, referenceError: valid ? null : '素材登记引用缺失或元数据不一致。' })
  }
  for (const asset of linked) {
    const id = asset.assetId ?? asset.id
    if (!id || catalog.has(id)) continue
    const ref = asset.objectRef
    const metadata = Object.fromEntries(['sha256', 'sizeBytes', 'mimeType'].map(key => [key, asset[key] ?? ref?.[key]]))
    const valid = matches(metadata, ref)
    catalog.set(id, { ...structuredClone(asset), id, ...metadata, objectRef: valid ? structuredClone(ref) : null, referenceError: valid ? null : '素材登记引用缺失或元数据不一致。' })
  }
  return [...catalog.values()]
}

export async function serveReferencedAsset({ repository, assetId, response }) {
  const asset = projectAssetCatalog(repository.getState()).find(item => item.id === assetId)
