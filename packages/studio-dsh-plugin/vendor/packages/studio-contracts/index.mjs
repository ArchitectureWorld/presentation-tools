...
export const ERROR_CODES = Object.freeze({
  ...
  WORKSPACE_SAVED_CONFLICT: 'local_saved_conflict',
...
export const REVIEW_SUBMISSION_TRANSITIONS = Object.freeze({
  ...
  dispatched: Object.freeze(['proposal_created', 'apply_failed', 'conflict', 'no_changes']),
  dispatch_failed: Object.freeze(['pending_dispatch']),
  apply_failed: Object.freeze(['pending_dispatch']),
  conflict: Object.freeze(['pending_dispatch']),
  no_changes: Object.freeze([]),
...
export const STUDIO_COMMAND_SCHEMA = Object.freeze({
  oneOf: [
    ...
    commandBranch('draft.list.update', { pageId: idSchema('page'), listItemId: idSchema('listItem'), content: { type: 'string' } }, ['pageId', 'listItemId', 'content']),
...
export const STUDIO_APPLY_COMMANDS_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    commands: { type: 'array', minItems: 0, maxItems: 500, items: STUDIO_COMMAND_SCHEMA },
    annotationResults: { type: 'array', maxItems: 500, items: {
      type: 'object', additionalProperties: false,
      required: ['annotationId', 'annotationVersion', 'status', 'reason', 'commandIds'],
      properties: {
        annotationId: studioIdSchema('annotation'),
        annotationVersion: { type: 'integer', minimum: 1 },
        status: { enum: ['completed', 'partial', 'unresolved'] },
        reason: { type: 'string', minLength: 1, maxLength: 4000 },
        commandIds: { type: 'array', uniqueItems: true, items: studioIdSchema('command') },
      },
    } },
  },
...
export function projectStateFromParts({ snapshot, currentRevision, operational = {}, ui = {} }) {
  ...
  return {
    ...
    designRuns: clone(operational.designRuns ?? []),
    layoutCandidates: clone(operational.layoutCandidates ?? []),
    designProtections: clone(operational.designProtections ?? []),
    designBatches: clone(operational.designBatches ?? []),
