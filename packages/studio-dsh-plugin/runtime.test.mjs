...
test('native DSH runtime binds isolated Report Studio projects to session ids and directly applies review commands', async () => {
  ...
  try {
    ...
    assert.equal(applied.currentRevision, submitted.submission.baseRevision + 1)
    ...
    assert.equal(repeated.currentRevision, applied.currentRevision)
    const sessionA = await runtime.getState('session-a')
    assert.equal(sessionA.proposals.length, 0)
    assert.equal(sessionA.reviewRuns[0].integrationState, 'accepted')
    assert.equal(sessionA.reviewRuns[0].resultProposalId, null)
    assert.equal(sessionA.reviewSubmissions[0].status, 'accepted')
