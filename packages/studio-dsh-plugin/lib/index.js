import {executeProductionApi,readDeliveryFile} from '../vendor/apps/studio-local/production-api.mjs'
import {assertDesignHostRequest} from '../vendor/apps/studio-local/host-request.mjs'
...
import { createDshReviewWorker } from './isolated-worker.js'
import { registerDesignTools, designSessionId } from './design-tools.js'
...
function registerTools(ctx, runtime) {
  ...
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        ...
        scope: {type:'string',enum:['design']},
        pageId: {type:'string'},
      },
      anyOf:[{required:['submissionId']},{required:['scope','pageId']}],
      additionalProperties: false,
    },
    output: toolOutput(),
    async execute(args, exec) {
      if(args.scope==='design')return outputJson(await (await runtime.designFor(designSessionId(exec))).context({sessionId:designSessionId(exec),pageId:args.pageId}))
  ...
  ctx.tools.register({
    ...
    description: '根据一个不可变 ReviewSubmission 提交结构化修改命令并直接应用到正式内容，经过 CAS 校验后产生新的 Revision，不创建审批 Proposal。无可执行修改时用 commands=[] 与原因结束任务但不标记批注完成。',
  ...
  return async (request, response) => {
    ...
    try {
      if (url.pathname.startsWith('/report-studio/api/')) {
        ...
        const productionMatch=url.pathname.match(/^\/report-studio\/api\/production\/(status|resume|cancel|retry|export)$/)
        if(productionMatch){
          const body=request.method==='GET'?Object.fromEntries([...url.searchParams].filter(([key])=>key!=='sessionId')):await readJson(request)
          return sendJson(response,200,await executeProductionApi({runtime,sessionId,operation:productionMatch[1],request,body}))
        }
        if(request.method==='GET'&&url.pathname==='/report-studio/api/delivery/file'){
          const result=await readDeliveryFile({repository:await runtime.repositoryFor(sessionId),deliveryId:url.searchParams.get('deliveryId'),name:url.searchParams.get('name')})
          response.statusCode=200;response.setHeader('content-type',result.mimeType);response.setHeader('content-disposition',`attachment; filename="${result.name}"`);response.setHeader('content-length',result.bytes.length);response.setHeader('cache-control','no-store');response.setHeader('x-content-type-options','nosniff');response.end(result.bytes);return
        }
        if(request.method==='GET'&&url.pathname==='/report-studio/api/design/image'){
          const result=await readDesignImage({runtime,sessionId,proposalId:url.searchParams.get('proposalId'),candidateId:url.searchParams.get('candidateId')})
          response.statusCode=200;response.setHeader('content-type',result.mimeType);response.setHeader('content-length',result.bytes.length);response.setHeader('cache-control','no-store');response.setHeader('x-content-type-options','nosniff');response.end(result.bytes);return
        }
        const designMatch=url.pathname.match(/^\/report-studio\/api\/design\/(start|context|progress|accept)$/)
        if(designMatch) {
          const body=request.method==='GET'?Object.fromEntries([...url.searchParams].filter(([key])=>key!=='sessionId')):await readJson(request)
          return sendJson(response,200,await executeDesignApi({runtime,sessionId,operation:designMatch[1],request,body}))
        }
        ...
        const layoutService = await runtime.layoutFor(sessionId)
        const standardProject = createStandardProjectService(repository, { layoutService })
        if (request.method === 'GET' && url.pathname === '/report-studio/api/health') {
          return sendJson(response, 200, {
            ...
            reviewWorkerConfigured: runtime.reviewWorkerConfigured,
            reviewWorkerMode: runtime.reviewWorkerMode,
            sessionId,
        ...
        if (layoutMatch) {
          try {
            if(request.method==='POST'&&layoutMatch.operation==='protection')assertDesignHostRequest(request)
        ...
        const confirmTaskMatch = url.pathname.match(/^\/report-studio\/api\/review-task\/([^/]+)\/confirm$/)
        if (request.method === 'POST' && confirmTaskMatch) return sendJson(response, 200, await runtime.confirmTask(sessionId, decodeURIComponent(confirmTaskMatch[1])))
...
export function apply(ctx, config = {}) {
  ...
  const timeoutMs = Number(config.reviewTimeoutMs ?? process.env.REPORT_STUDIO_AGENT_TIMEOUT_MS ?? 120_000)
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) throw new Error('Invalid Report Studio review timeout')
  const url = config.agentUrl || process.env.REPORT_STUDIO_AGENT_URL || ''
  const localWorker = createDshReviewWorker(ctx, { timeoutMs })
  if (!url && !localWorker.configured && config.allowNativeReview !== true) {
    throw new Error('Report Studio isolated review worker is unavailable; refusing to route annotations into the parent DSH Session')
  }
  const agentBridge = url ? createAgentBridge({ url, timeoutMs }) : localWorker.configured ? localWorker : undefined
  const runtime = createStudioDshRuntime({ dataRoot: config.dataDir, sessions: ctx.sessions, agentBridge, leaseMs: timeoutMs })
  const nativeListeners = [
    ctx.on?.('session/event', (session, event) => runtime.handleNativeSessionEvent(session, event)),
    ctx.on?.('session/disposed', session => runtime.handleNativeSessionEvent(session, { type: 'session/disposed' }, { disposed: true })),
  ]
  ...
  ctx.inject?.(['preplanning'], scope => {
    ...
    if (visualBridgeReady(preplanning)) {
      return scope.effect(() => runtime.bindDesignVisual(preplanning), 'report-studio: Pre visual bridge')
    }
  })
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      'studio_apply_commands 在批注提交后自动应用所有经过严格校验的修改并创建新的 Revision；不创建 Proposal，不等待确认。',
      'Presentation is an editing, rendering, checking and persistence tool in the current DSH Session.',
      'Batch completion is based on current source hashes and saved PNG evidence, not Agent-reported success. Local exceptions use studio_report_design_exception and do not stop unrelated pages. studio_export_delivery produces checked image-based HTML/PDF/PPTX plus edit sources, not object-editable PPTX.',
      'Report writing, content strategy and layout Skill belong to Pre-design. DSH remains the only Agent runtime and model authority.',
      'Read studio_get_context or studio_get_layout_context before editing. Respect the task scope, protected pages, source hashes and current Revision.',
      'User-requested editing is applied directly through validated tools; no Proposal acceptance step is required. Internal record IDs are compatibility handles, not approval requests.',
      'studio_prepare_layout_candidate, studio_render_layout_preview and studio_submit_layout_review provide the controlled rendering and save operations.',
      'studio_generate_design_visual and studio_adopt_design_visual provide image generation/link operations within explicit paid-generation permission.',
      'If linking or the Pre receipt is interrupted, studio_resume_design_visual resumes the same runId/pageId/sourceStateHash/requestId without regenerating the image.',
      'For annotation tasks, use studio_apply_commands with the supplied submissionId and per-annotation outcomes. Unchanged user instructions only may be marked completed.',
    ].join('\n'),
  })
  ctx.effect(() => {
    ...
    return async () => {
      unregister?.()
      for (const off of nativeListeners) off?.()
      await runtime.close()
