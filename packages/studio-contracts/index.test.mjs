...
test('review lifecycle contract exposes the closed monotonic state graph', () => {
  assert.deepEqual(REVIEW_RUN_INTEGRATION_STATES, [
    'pending_dispatch', 'dispatched', 'dispatch_failed', 'apply_failed', 'conflict', 'no_changes', 'proposal_created', 'accepted', 'rejected', 'stale',
  ])
  assert.deepEqual(REVIEW_SUBMISSION_TRANSITIONS, {
    pending_dispatch: ['dispatched', 'dispatch_failed'],
    dispatched: ['proposal_created', 'apply_failed', 'conflict', 'no_changes'],
    dispatch_failed: ['pending_dispatch'],
    apply_failed: ['pending_dispatch'],
    conflict: ['pending_dispatch'],
    no_changes: [],
