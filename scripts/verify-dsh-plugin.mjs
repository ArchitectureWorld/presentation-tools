...
import {registerDesignTools} from '../packages/studio-dsh-plugin/lib/design-tools.js'
...
for(const name of ['studio_get_layout_context','studio_prepare_layout_candidate','studio_render_layout_preview','studio_submit_layout_review','studio_read_design_image','studio_generate_design_visual','studio_adopt_design_visual','studio_prepare_design_content']){
  const tool=designTools.find(row=>row.name===name)
  assert.ok(tool?.execute&&tool?.output?.render,`missing callable ${name}`)
  assert.equal(tool.parameters.additionalProperties,false)
  assert.equal(Object.hasOwn(tool.parameters.properties,'allowApply'),false)
}
