...
let server
try {
  const imported = await readStandardProject(workspace, { putBlob: repository.putBlob })
  ...
  const html = `<!doctype html><meta charset="utf-8"><style>${appCss}\n${layoutCss}</style><body>${markup}<script>
    window.addEventListener('load', async () => {
      ...
      const textOverflow = [], outsideCanvas = [], brokenImages = [];
      ...
      document.body.setAttribute('data-layout-audit', encodeURIComponent(JSON.stringify({textOverflow,outsideCanvas,brokenImages})));
    });
  </script></body>`
  ...
  console.log(JSON.stringify(summary, null, 2))
  assert.equal(failures.length + browser.textOverflow.length + browser.outsideCanvas.length + browser.brokenImages.length, 0, 'layout verification failed')
  }
} finally {
