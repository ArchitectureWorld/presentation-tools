...
async function tar(commandArgs, encoding = 'utf8') {
  const [operation, archivePath, ...members] = commandArgs
  const archive = resolve(archivePath)
  // GNU tar interprets Windows drive prefixes as remote hosts. A cwd-local,
  // explicitly relative archive operand works with both GNU and BSD tar.
  const result = await execFileAsync('tar', [operation, `./${basename(archive)}`, ...members], {
    cwd: dirname(archive), encoding, maxBuffer: 32 * 1024 * 1024,
  })
